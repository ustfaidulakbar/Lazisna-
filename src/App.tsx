      ...
      <main className="w-full max-w-[480px] bg-slate-50 h-[100dvh] shadow-2xl relative flex flex-col justify-between overflow-hidden border-x border-slate-200/40">
        ...
        <div className="flex-1 overflow-y-auto pb-24">
          ...
          {isAdminActive ? (
            ...
            <>
              ...
              {/* 1. HOME TAB */}
              {activeTab === "beranda" && (
                <div className="space-y-5 px-5 mt-[-24px] relative z-10">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <button
                        onClick={() => setActiveTab("ustadz-ai")}
              ...
              {/* 2. ZAKAT CALCULATOR TAB */}
              {activeTab === "kalkulator" && (
                <div className="p-5 space-y-4">
                  <div className="text-left space-y-1 mb-2">
                    <h2 className="text-lg font-black text-slate-800">Kalkulator Ziswaf</h2>
                    <p className="text-xs text-slate-400 leading-snug">Gunakan kalkulator syariah ini untuk menghitung nishab zakat penghasilan bulanan serta simpanan harta tahunan Anda.</p>
                  </div>
                  <ZakatCalculator 
        ...
        {!isCheckoutOpen && !isAdminActive && (
          <nav className="absolute bottom-0 inset-x-0 bg-white/90 backdrop-blur-md border-t border-slate-200/80 rounded-t-2xl px-3 py-2 flex justify-around items-center z-40 shadow-lg shadow-emerald-900/10">
            ...
            <button
              onClick={() => setActiveTab("ustadz-ai")}
              className={`flex flex-col items-center gap-1 p-2 text-[10px] font-bold transition-all relative ${
                activeTab === "ustadz-ai" ? "text-emerald-600 scale-105" : "text-slate-400 hover:text-slate-600"
              }`}
            >
              <Sparkles className="w-5 h-5 stroke-[2] text-amber-500 fill-amber-500/20" />
              <span>Tanya AI</span>
              <span className="absolute top-1.5 right-1 w-1.5 h-1.5 bg-yellow-400 rounded-full animate-ping" />
            </button>
